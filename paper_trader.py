...
class PaperTrader:
    def __init__(self, exchange_name="bitrue", amount=10.0, threshold=0.01, scans=10):
        ...
        # Dreiecks-Routen inklusive DFI (DeFiChain)
        self.routes = [
            ["USDT", "DFI", "BTC", "USDT"],
            ["USDT", "DFI", "ETH", "USDT"],
            ["USDT", "ETH", "BTC", "USDT"],
    ...
    def run(self):
        results = []
        print(f"🔎 Starte {self.total_scans} Dreiecks-Arbitrage-Scans auf {self.exchange_name.upper()}...")
        
        for scan in range(1, self.total_scans + 1):
            print(f"\n--- Scan {scan}/{self.total_scans} ---")
